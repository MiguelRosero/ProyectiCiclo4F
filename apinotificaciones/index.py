...
def send_email():
    ...
    #Creacion del objeto del mensaje a enviar
    message = Mail(
    ...
    try:
       #aqui se envia el archivo
        sg = SendGridAPIClient(env['SENDGRID_API_KEY'])
        response = sg.send(message)
        print(response.status_code)
        print(response.body)
        print(response.headers)
        return "send success" #saldrá esto si es correcto
    except Exception as e:
        print(e)
        return "error" # Saldrá esto si es erroneo el envio
